...
@njit(fastmath=True)
def iterator_n(x, y, z, coeffs, tmax):
...
def itdata(coeffs,tmax,colorstep,x0,y0,z0):
	...
	xl, yl, zl = iterator_n(x, y, z, np.array(coeffs), tmax)
...
coeff_sliders = []
for i in range(30):
	coeff_sliders_ax.append(fig.add_axes([1.1*pos.x1, pos.y1 - (0.02*i), pos.width/3., 0.01]))
	if i < 10:
